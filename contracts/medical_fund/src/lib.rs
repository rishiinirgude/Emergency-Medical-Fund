...
//!
//! - Campaigns are created for a patient + verified hospital.
//! - Donors send XLM and receive CARE tokens.
//! - Funds released per milestone after 2-of-3 multi-sig approval
//!   (creator + hospital + admin, 2-of-3 required).
//!
//! Inter-contract calls:
//!   • HospitalVerification.is_verified() — on createCampaign
...
pub enum DataKey {
    Admin,
    HvContract,   // HospitalVerification contract address
    CtContract,   // CareToken contract address
    XlmToken,     // Native XLM token contract address
    CampaignCount,
    Campaign(u32),
    MilestoneCount(u32),
    Milestone(u32, u32),          // (campaign_id, milestone_index)
    Donation(u32, Address),       // (campaign_id, donor)
    Donors(u32),
    HasApproved(u32, u32, Address), // (campaign_id, milestone_index, approver)
}
...
#[contractimpl]
impl MedicalFund {
    /// Initialize with admin, HospitalVerification contract, CareToken contract,
    /// and the native XLM token contract address.
    pub fn initialize(
        env: Env,
        admin: Address,
        hv_contract: Address,
    ...
    pub fn create_campaign(
        ...
        // Inter-contract call — verify hospital is whitelisted
        let hv: Address = env.storage().instance().get(&DataKey::HvContract).unwrap();
        let is_verified: bool = env.invoke_contract(
            &hv,
            &symbol_short!("is_verifid"),
            soroban_sdk::vec![&env, hospital.clone().into()],
        );
        if !is_verified {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::MilestoneCount(campaign_id), &0_u32);
        env.storage()
            .persistent()
            .set(&DataKey::Donors(campaign_id), &Vec::<Address>::new(&env));
        env.storage()
            .instance()
    ...
    /// Donate XLM to a campaign. Mints CARE tokens as reward.
    /// Donor must have approved the token transfer beforehand.
    pub fn donate(env: Env, donor: Address, campaign_id: u32, amount: i128) {
        ...
        // Inter-contract call — mint CARE reward tokens
        let ct: Address = env.storage().instance().get(&DataKey::CtContract).unwrap();
        let _: () = env.invoke_contract(
            &ct,
            &symbol_short!("mint_rwrd"),
            soroban_sdk::vec![&env, donor.clone().into(), amount.into()],
        );

        // Compute care tokens for event (1000 CARE per 1 XLM)
    ...
    /// Add a milestone to a campaign. Creator only.
    pub fn add_milestone(env: Env, campaign_id: u32, amount: i128) {
        ...
        // Ensure total milestones don't exceed target
        let count: u32 = env
            .storage()
            .persistent()
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Milestone(campaign_id, count), &milestone);
        env.storage()
            .persistent()
            .set(&DataKey::MilestoneCount(campaign_id), &(count + 1));

        env.events()
            .publish((EVT_MILESTONE_ADDED,), (campaign_id, count, amount));
    }

    /// Approve a milestone. Creator, hospital, or admin (2-of-3 required).
    pub fn approve_milestone(env: Env, approver: Address, campaign_id: u32, milestone_index: u32) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Milestone(campaign_id, milestone_index), &milestone);
    ...
    /// Release funds for an approved milestone to the hospital.
    pub fn release_milestone(env: Env, caller: Address, campaign_id: u32, milestone_index: u32) {
        ...
        if !milestone.approved {
            panic!("MF: milestone not approved");
        }
        if milestone.released {
            panic!("MF: already released");
        }
    ...
    /// Close a campaign. Creator or admin only.
    pub fn close_campaign(env: Env, caller: Address, campaign_id: u32) {
        ...
        if caller != campaign.creator && caller != admin {
            panic!("MF: not authorized");
        }
        if !campaign.active {
            panic!("MF: already closed");
        }
        ...
        env.events()
            .publish((EVT_CAMPAIGN_CLOSED,), (campaign_id, env.ledger().timestamp()));
    }
    ...
    pub fn has_approved(env: Env, campaign_id: u32, milestone_index: u32, approver: Address) -> bool {
        env.storage()
            .persistent()
            .get(&DataKey::HasApproved(campaign_id, milestone_index, approver))
            .unwrap_or(false)
    }
...
mod tests {
    ...
    fn setup() -> TestEnv {
        ...
        xlm_client.mint(&donor, &100_0000000_i128); // 100 XLM
    ...
    fn test_create_campaign() {
        let t = setup();
        let id = t.mf_client.create_campaign(
            &t.creator,
            &t.patient,
            &t.hospital,
    ...
    fn test_donate_updates_balance_and_mints_care() {
        ...
        t.mf_client.donate(&t.donor, &0, &1_0000000_i128); // 1 XLM
        let c = t.mf_client.get_campaign(&0);
    ...
    fn test_milestone_full_flow() {
        ...
        let m2 = t.mf_client.get_milestone(&0, &0);
        assert!(m2.released);
    }
